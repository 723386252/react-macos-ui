...
const WindowWrapper = styled.div<WindowWrapperProps>`
  ...
  border-radius: ${(props) =>
    props.$isFullScreen ? '0' : 'var(--window-radius)'};
  box-shadow: ${(props) =>
    props.$isFullScreen
      ? 'none'
      : props.$focused
...
const WindowHeader = styled.div<{
  ...
  right: 0;
  height: ${(props) =>
    props.$isFullScreen ? FULLSCREEN_HEADER_HEIGHT : NORMAL_HEADER_HEIGHT};
...
const WindowContent = styled.div<{ $isFullScreen: boolean }>`
  height: ${(props) =>
    props.$isFullScreen ? '100vh' : `calc(100% - ${NORMAL_HEADER_HEIGHT})`};
  margin-top: ${(props) =>
    props.$isFullScreen ? FULLSCREEN_HEADER_HEIGHT : '0'};
  flex: 1;
  overflow: hidden;
`;
...
export const Window = forwardRef<WindowRef, WindowProps>(
  (
    ...
    if (!defaultPosition) {
      defaultPosition = { x: window.innerWidth / 2 - defaultWidth / 2, y: window.innerHeight / 2 - defaultHeight / 2 };
    }
    ...
    const mouse = useMouse(windowRef as React.RefObject<Element>);
    ...
    const [minimizePosition, setMinimizePosition] = useState<
    ...
    const handleDrag = (e: any, d: { x: number; y: number }) => {
      flushSync(() => {
        if (isMaximized && dragStartMousePos) {
          ...
          const newX =
            e.clientX - windowWidth * (1 - dragStartMousePos.relativeX);
    ...
    // 优化计算最小化位置的函数
    const calculateMinimizePosition = useCallback(
    ...
    const handleMinimize = useCallback(
      (targetElement?: HTMLElement) => {
        ...
        if (onMinimize) {
          targetElement = onMinimize() ?? targetElement;
        }
    ...
    const openWindow = useCallback(() => {
      setVisible(true);
      setCloseType(WindowCloseType.None);
    }, []);
    ...
    return useMemo(
      () =>
        fixed ? (
          <FixedContainer
            $fixed={fixed}
            style={{ zIndex: windowContext.zIndex }}>
            {windowContent}
          </FixedContainer>
        ) : (
          windowContent
